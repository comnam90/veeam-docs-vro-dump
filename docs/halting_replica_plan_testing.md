...
product: "vro"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/vro/userguide/halting_replica_plan_testing.html"
...
The Halt action interrupts plan testing. You may need to halt plan testing, for example, if you need to fix some environment-related issues and then [proceed with testing later](resuming_replica_plan_testing.md) (in this case, VM replicas will still continue to run). Or you may need to stop the testing process completely, for example, if you no longer need to test the selected replica plan (in this case, VM replicas will be reverted to the latest restore point).
